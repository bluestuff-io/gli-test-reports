...
| | |
|---|---|
| **Report Number** | `BS-SLOTFORGE-20260731-144CD595` |
...
|---|---|---|---|
| **3.2.2** | 🔴 FAIL | 240,000 | p = 2.79e-5 |
| **3.2.3** | 🟢 PASS | 240,000 | p = 9.28e-1 |
| **3.2.4** | 🟢 PASS | 180,000 | p = 7.09e-1 |
| **4.7.1** | 🟡 WARN | 120,000 | — |
...
Shared RNG source (crypto-backed, used by every engine on this template). float() scaled to 32 bins, 2 independent draws combined by Fisher's method. 7 of the 7 tests named in §3.2.2 applied. 1 rejected at the 99% family-wise confidence level: duplicates (p = 2.788e-5, threshold 1.429e-3).

Evaluated collectively at 99% family-wise confidence (holm-bonferroni). Fisher combined p = 5.386e-2.

<details><summary>Per-test results</summary>

| Test | GLI §3.2.2 | Statistic | p-value | Applied |
|---|---|---|---|---|
| chi-square | (a) | 0.879 | 9.275e-1 | yes |
| overlaps | (b) | 2.146 | 7.090e-1 | yes |
| coupon-collector | (c) | 1.664 | 7.973e-1 | yes |
| runs | (d) | 2.650 | 6.179e-1 | yes |
| interplay-correlation | (e) | 0.000 | 1.000e+0 | yes |
| serial-correlation | (f) | 0.985 | 9.121e-1 | yes |
| duplicates | (g) | 26.273 | 2.788e-5 | yes |

</details>

### 3.2.3 — 🟢 PASS

RNG output is uniformly distributed (float 32-bin: Fisher chi-square = 0.88, p = 0.9275; int 64-range: p = 0.5768).
...
|---|---|---|---|---|
| chi-square | (a) | 0.879 | 9.275e-1 | yes |
| chi-square | (a) | 60.199 | 5.768e-1 | yes |

</details>

### 3.2.4 — 🟢 PASS

Independence of the shared RNG (serial, overlaps, interplay) and of each engine's per-round outcomes (slotforge-mvp). 4 of the 7 tests named in §3.2.2 applied. None rejected at the 99% family-wise confidence level (Holm–Bonferroni; Fisher combined p = 9.989e-1).

Evaluated collectively at 99% family-wise confidence (holm-bonferroni). Fisher combined p = 9.989e-1.
...
| interplay-correlation | (e) | 0.000 | 1.000e+0 | yes |
| serial-correlation | (f) | 0.985 | 9.121e-1 | yes |
| outcome-independence:slotforge-mvp | (f) | -0.934 | 1.000e+0 | yes |
...
Return-to-player measured per registered engine by driving play() with the template RNG:
  • example: no payouts observed over 60,000 spins — stub or non-payout engine; nothing to certify.
  • slotforge-mvp: RTP 94.099% ±8.915pp over 60,000 spins (hit 34.89%, max 385.0x) — provisional (widen sample).
...
```bash
node src/cli.ts --engine=slotforge --repo=D:\Projects\smokestack-original-games\BackendEngines\slotrocket-5c5a --samples=40000 --spins=60000 --replicates=2 --seed=pdf
```
...
| Seed | `pdf` |
| Started | 2026-07-31T09:44:10.918Z |
| Finished | 2026-07-31T09:44:12.780Z |
...
Report `BS-SLOTFORGE-20260731-144CD595` · Generated 2026-07-31T09:44:12.780Z

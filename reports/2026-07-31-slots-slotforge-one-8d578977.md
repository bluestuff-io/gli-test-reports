...
| | |
|---|---|
| **Report Number** | `BS-SLOTS-20260731-8D578977` |
...
| **Result Summary** | 2 passed · 1 failed · 1 warnings · 0 manual · 0 source findings |
...
This build did not pass every check. Details and reproduction steps are below.
...
|---|---|---|---|
| **3.2.2** | 🟢 PASS | 600,000 | p = 2.23e-1 |
| **3.2.3** | 🟢 PASS | 600,000 | p = 4.52e-1 |
| **3.2.4** | 🔴 FAIL | 320,000 | p = 1.17e-162 |
| **4.7.1** | 🟡 WARN | 400,000 | — |
...
Shared RNG source (crypto-backed, used by every engine on this template). float() scaled to 32 bins, 5 independent draws combined by Fisher's method. 7 of the 7 tests named in §3.2.2 applied. None rejected at the 99% family-wise confidence level (Holm–Bonferroni; Fisher combined p = 6.768e-1).

Evaluated collectively at 99% family-wise confidence (holm-bonferroni). Fisher combined p = 6.768e-1.

<details><summary>Per-test results</summary>

| Test | GLI §3.2.2 | Statistic | p-value | Applied |
|---|---|---|---|---|
| chi-square | (a) | 9.866 | 4.524e-1 | yes |
| overlaps | (b) | 12.105 | 2.781e-1 | yes |
| coupon-collector | (c) | 13.019 | 2.226e-1 | yes |
| runs | (d) | 5.312 | 8.694e-1 | yes |
| interplay-correlation | (e) | 8.254 | 6.040e-1 | yes |
| serial-correlation | (f) | 10.125 | 4.296e-1 | yes |
| duplicates | (g) | 8.193 | 6.100e-1 | yes |

</details>

### 3.2.3 — 🟢 PASS

RNG output is uniformly distributed (float 32-bin: Fisher chi-square = 9.87, p = 0.4524; int 64-range: p = 0.4875).
...
|---|---|---|---|---|
| chi-square | (a) | 9.866 | 4.524e-1 | yes |
| chi-square | (a) | 62.685 | 4.875e-1 | yes |

</details>

### 3.2.4 — 🔴 FAIL

Independence of the shared RNG (serial, overlaps, interplay) and of each engine's per-round outcomes (slotforge-mvp). 4 of the 7 tests named in §3.2.2 applied. 1 rejected at the 99% family-wise confidence level: outcome-independence:slotforge-mvp (p = 1.166e-162, threshold 2.500e-3).

Evaluated collectively at 99% family-wise confidence (holm-bonferroni). Fisher combined p = 7.484e-157.
...
| interplay-correlation | (e) | 8.254 | 6.040e-1 | yes |
| serial-correlation | (f) | 10.125 | 4.296e-1 | yes |
| outcome-independence:slotforge-mvp | (f) | 27.263 | 1.166e-162 | yes |
...
Return-to-player measured per registered engine by driving play() with the template RNG:
  ...
  • slotforge-mvp: RTP 91.560% ±5.872pp over 200,000 spins (hit 35.06%, max 2520.7x) — provisional (widen sample).
...
```bash
node src/cli.ts --engine=slots --repo=D:\Projects\Devops\MCP\mcp-server\.repo-cache\games\bluestuff-io__smokestack-original-games\BackendEngines\slotrocket-5c5a --samples=100000 --spins=200000 --replicates=5 --seed=gli-1785491773484
```
...
| Seed | `gli-1785491773484` |
| Started | 2026-07-31T09:56:13.618Z |
| Finished | 2026-07-31T09:56:18.451Z |
...
Report `BS-SLOTS-20260731-8D578977` · Generated 2026-07-31T09:56:18.451Z

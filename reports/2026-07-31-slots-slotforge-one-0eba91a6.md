...
| | |
|---|---|
| **Report Number** | `BS-SLOTS-20260731-0EBA91A6` |
...
|---|---|---|---|
| **3.2.2** | 🟢 PASS | 600,000 | p = 3.07e-1 |
| **3.2.3** | 🟢 PASS | 600,000 | p = 7.10e-1 |
| **3.2.4** | 🟢 PASS | 320,000 | p = 4.25e-1 |
| **4.7.1** | 🟡 WARN | 400,000 | — |
...
Shared RNG source (crypto-backed, used by every engine on this template). float() scaled to 32 bins, 5 independent draws combined by Fisher's method. 7 of the 7 tests named in §3.2.2 applied. None rejected at the 99% family-wise confidence level (Holm–Bonferroni; Fisher combined p = 9.199e-1).

Evaluated collectively at 99% family-wise confidence (holm-bonferroni). Fisher combined p = 9.199e-1.

<details><summary>Per-test results</summary>

| Test | GLI §3.2.2 | Statistic | p-value | Applied |
|---|---|---|---|---|
| chi-square | (a) | 7.168 | 7.095e-1 | yes |
| overlaps | (b) | 10.181 | 4.248e-1 | yes |
| coupon-collector | (c) | 10.186 | 4.244e-1 | yes |
| runs | (d) | 6.454 | 7.757e-1 | yes |
| interplay-correlation | (e) | 3.638 | 9.622e-1 | yes |
| serial-correlation | (f) | 5.436 | 8.602e-1 | yes |
| duplicates | (g) | 11.681 | 3.070e-1 | yes |

</details>

### 3.2.3 — 🟢 PASS

RNG output is uniformly distributed (float 32-bin: Fisher chi-square = 7.17, p = 0.7095; int 64-range: p = 0.4808).
...
|---|---|---|---|---|
| chi-square | (a) | 7.168 | 7.095e-1 | yes |
| chi-square | (a) | 62.874 | 4.808e-1 | yes |

</details>

### 3.2.4 — 🟢 PASS

Independence of the shared RNG (serial, overlaps, interplay) and of each engine's per-round outcomes (slotforge-mvp). 4 of the 7 tests named in §3.2.2 applied. None rejected at the 99% family-wise confidence level (Holm–Bonferroni; Fisher combined p = 9.781e-1).

Evaluated collectively at 99% family-wise confidence (holm-bonferroni). Fisher combined p = 9.781e-1.
...
| interplay-correlation | (e) | 3.638 | 9.622e-1 | yes |
| serial-correlation | (f) | 5.436 | 8.602e-1 | yes |
| outcome-independence:slotforge-mvp | (f) | -0.821 | 1.000e+0 | yes |
...
Return-to-player measured per registered engine by driving play() with the template RNG:
  ...
  • slotforge-mvp: RTP 95.313% ±5.358pp over 200,000 spins (hit 35.04%, max 1145.2x) — provisional (widen sample).
...
```bash
node src/cli.ts --engine=slots --repo=D:\Projects\Devops\MCP\mcp-server\.repo-cache\games\bluestuff-io__smokestack-original-games\BackendEngines\slotrocket-5c5a --samples=100000 --spins=200000 --replicates=5 --seed=gli-1785490407469
```
...
| Seed | `gli-1785490407469` |
| Started | 2026-07-31T09:33:27.615Z |
| Finished | 2026-07-31T09:33:33.360Z |
...
Report `BS-SLOTS-20260731-0EBA91A6` · Generated 2026-07-31T09:33:33.360Z
